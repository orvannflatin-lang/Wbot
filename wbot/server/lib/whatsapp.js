    ...
    try {
        ...
        if (encodedId.startsWith('WBOT-MD_V4_')) {
            ...
            const { data: userSettings, error } = await supabase
                .from('user_settings')
                .select('session_dump')
                .eq('user_id', sessionUuid) // sessionUuid is the user_id uuid
                .single();

            if (error || !userSettings?.session_dump) throw new Error("Session dump not found in Supabase.");

            creds = userSettings.session_dump;
            console.log(`[WBOT] ✅ V4 Session Dump retrieved.`);
...
export const startWhatsAppSession = async (sessionId, io, isReset = false) => {
    try {
        ...
        const sock = makeWASocket({
            ...
            markOnlineOnConnect: true, // Force "Online" status for stability
                                ...
                                if (fs.existsSync(path.join(authFolder, 'creds.json'))) {
                                    ...
                                    if (supabase && process.env.SUPABASE_URL) {
                                        try {
                                            ...
                                            console.log('[WBOT] Supabase error, falling back to V2 session string');
                                        }
                                    }
        ...
        sock.ev.on('messages.upsert', async ({ messages, type }) => {
            ...
            if (settingsId && supabase && process.env.SUPABASE_URL) {
                try {
                    ...
                    console.log('[WBOT] Using default config (Supabase not available)');
                }
            }
